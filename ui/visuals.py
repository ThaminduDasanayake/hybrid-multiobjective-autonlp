...
def plot_pareto_front_2d(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=[
                f"{row['vectorizer']}-{row['model']}" for _, row in df_all.iterrows()
            ],
...
def plot_search_history(search_history: List[Dict[str, Any]], key: str = None):
    ...
    with col1:
        ...
        st.plotly_chart(
            fig_latency, width="stretch", key=f"{key}_latency" if key else None
        )
    ...
    with col2:
        ...
        st.plotly_chart(
            fig_interp, width="stretch", key=f"{key}_interp" if key else None
        )


def show_solutions_table(
    solutions: List[Dict[str, Any]],
    pareto_front: List[Dict[str, Any]],
    knee_point: Dict[str, Any] = None,
):
    ...
    display_df["interpretability"] = display_df["interpretability"].apply(
        lambda x: f"{x:.4f}"
    )
